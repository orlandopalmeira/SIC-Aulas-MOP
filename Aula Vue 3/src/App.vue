...
export default {
  ...
  methods: {
    ...
    addGame(newGame) {
      const newg = {
        id: this.games.map(game => game.id).reduce((a, b) => Math.max(a, b), 0) + 1,
        name: newGame.name,
        year: newGame.year,
        platform: newGame.platform,
        user: this.userName
      };
      const newgJSON = JSON.stringify(newg, (k, v) => (k === "id") ? new String(v) : v); // Convert id to string
      alert(newgJSON);
      ...
      fetch(url, request)
        ...
        .then(data => this.games.push(data))
